...
1主2从 Redis Sentinel 自动故障转移集群，纯 Helm Chart，零 Operator。
...
```bash
# 快速多实例测试（部署 → 验证 → failover → 清理）
./test.sh                                # 默认实例 (instance=redis-test)
./test.sh my-app                         # 指定实例名
./test.sh my-app middleware              # 指定 namespace

# 单独运行某个阶段
./test.sh my-app redis verify            # 仅验证
./test.sh my-app redis failover          # 仅 failover 测试
./test.sh my-app redis cleanup           # 仅清理

# 完整稳定性测试（9 个故障场景 + RBAC 验证）
./test.sh my-app redis stability
```
...
# 脚本方式
./test.sh <instance> <namespace> cleanup
```
