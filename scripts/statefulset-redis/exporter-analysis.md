...
1. **指标采集**：定期查询 Redis 的 INFO 命令，提取关键指标
...
```bash
# exporter 采集不影响 Redis 主进程
redis-cli INFO  # 只读操作，不影响写性能
```

**为什么稳定**：
- INFO 命令是只读操作，不影响 Redis 写性能
- 采集频率可配置（默认 15s）
- 即使 exporter 挂了，Redis 主进程不受影响
...
| Redis 不可达 | 指标中标记错误，exporter 继续运行 | ✅ 不影响 Redis |
